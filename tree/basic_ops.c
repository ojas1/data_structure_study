...
*/
	...
	TNode* root = NULL;
	_status_code status = insert(&root, data[0]);
	status = insert(&root, data[1]);
	status = insert(&root, data[2]);
	status = insert(&root, data[3]);
	_status_code del_stat = delete_with_id(&root, data[3].id);
	printf("\ndeletion status: %d\n", del_stat);
	return 0;
}